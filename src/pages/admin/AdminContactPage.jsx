...
export function AdminContactPage() {
  ...
  async function handleAdd() {
    const { count } = await supabase.from('contact_info').select('id', { count: 'exact', head: true });
    const { data, error } = await supabase
      .from('contact_info')
      .insert({ platform: 'Email', label: 'Email', value: '', icon: '✉', visible: true, display_order: count ?? 0 })
      .select()
      .single();
    if (error) setFeedback({ type: 'error', message: error.message });
  ...
  async function handleDelete(id) {
    if (!confirmAction('Delete this contact entry?')) return;
  ...
  async function handleSave() {
    ...
    setSaving(false);
    setFeedback(err ? { type: 'error', message: err.message } : { type: 'success', message: 'Contact info saved.' });
  }

  async function move(index, dir) {
    const j = index + dir;
    if (j < 0 || j >= rows.length) return;
    const err = await swapOrder(supabase, 'contact_info', rows[index], rows[j]);
            ...
            <div>
              <div className="admin-grid-2">
                <AdminField label="Platform">
                  <select className="admin-select" value={r.platform} onChange={(e) => updateLocal(r.id, { platform: e.target.value })}>
                    {PLATFORMS.map((p) => (
                ...
                <AdminField label="Icon (emoji or text)">
                  <input className="admin-input" value={r.icon ?? ''} onChange={(e) => updateLocal(r.id, { icon: e.target.value })} />
                </AdminField>
                <AdminField label="Label">
                  <input className="admin-input" value={r.label ?? ''} onChange={(e) => updateLocal(r.id, { label: e.target.value })} />
                </AdminField>
                <AdminField label="Value">
                  <input className="admin-input" value={r.value ?? ''} onChange={(e) => updateLocal(r.id, { value: e.target.value })} />
                </AdminField>
              </div>
